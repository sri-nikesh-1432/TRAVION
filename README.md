...
- **World-scale search, instant India planning.** Source and destination are live global location fields backed by a bundled index of every Indian state, district, city and town (4,600+ records with real coordinates) plus live Google Places autocomplete for the rest of the world. **Every India-to-India journey plans instantly**: destinations with published verified packages (stays, dining, attractions, transport) use the fully verified engine, and every other Indian pair — Hyderabad, Kodaikanal, Leh, any town — is planned by the India-wide estimate engine using the traveller's real coordinates, real distance-based corridors and clearly marked `estimate` content. No train numbers, flight codes, hotel names or attraction claims are ever invented; live specifics are confirmed by the trip assistant before travel. Destinations outside India still return an honest structured refusal with reachable alternatives until world coverage ships.
