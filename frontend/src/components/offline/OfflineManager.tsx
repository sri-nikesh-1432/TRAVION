      ...
      >
        <div className="flex items-center justify-between pb-4 border-b border-charcoal-100 mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-lg font-bold text-charcoal-900">Download Offline Trip Package</h3>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="space-y-2.5 mb-6">
          {checklistItems.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2.5 text-xs font-semibold text-charcoal-700">
              <CheckCircle2 className={`w-4 h-4 ${isCached ? 'text-emerald-500' : 'text-charcoal-300'}`} />
              <span>{item}</span>
            </div>
          ))}
        </div>
        ...
        {isDownloading && (
          <div className="mb-6">
            <div className="flex justify-between text-xs font-bold text-charcoal-600 mb-1">
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2.5 rounded-xl border border-charcoal-200 text-charcoal-600 font-semibold text-xs hover:bg-ivory-50 transition-colors"
          >
            Close
          </button>
