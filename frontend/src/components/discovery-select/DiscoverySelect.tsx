...
import { BadgeCheck, MapPin, Star, BedDouble, Utensils, Mountain, ArrowRight, Landmark } from 'lucide-react';
...
export const DiscoverySelect: React.FC<DiscoverySelectProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      {!catalog && !loadError && (
        <div className="py-16 text-center text-[13px] font-bold text-slate-400">Loading verified places…</div>
      )}
            ...
            <div className="grid sm:grid-cols-3 gap-3">
              {catalog.stays.map((stay) => (
                <div key={stay.name} className="p-4 rounded-2xl border border-slate-200 bg-white">
                  <p className="text-[10px] font-black uppercase tracking-wide text-travion-700">{stay.tier}</p>
                  <p className="text-[13px] font-extrabold text-slate-900 leading-snug mt-0.5">{stay.name}</p>
                  <div className="mt-2 flex items-center gap-2 text-[10.5px] font-bold text-slate-400">
                    <span>₹{stay.price_per_night.toLocaleString('en-IN')}/night</span>
