...
export const JourneyTimeline: React.FC<JourneyTimelineProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white/80 backdrop-blur-xl rounded-3xl border border-charcoal-200/70 shadow-soft overflow-hidden">
      {/* Header: route + live status + progress */}
      <div className="px-5 pt-4 pb-3 border-b border-charcoal-100 bg-gradient-to-b from-white to-ivory-50/60">
        <div className="flex items-center justify-between gap-2 mb-1">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-[13px] font-extrabold text-charcoal-900 truncate">
                {tripSource || itinerary.trip_id.slice(0, 8)} <span className="text-charcoal-300 mx-0.5">→</span> {tripDestination || 'Destination'}
              </p>
          ...
          <div className={`flex items-center gap-1.5 shrink-0 rounded-full px-2.5 py-1 text-[10px] font-bold border ${
            gpsLive ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-sand-100 border-charcoal-200 text-charcoal-500'
          }`}>
            <span className="relative flex w-1.5 h-1.5">
              {gpsLive && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-60" />}
              <span className={`relative inline-flex rounded-full w-1.5 h-1.5 ${gpsLive ? 'bg-emerald-500' : 'bg-sand-400'}`} />
            </span>
        ...
        <div className="mt-3 flex items-center gap-2.5">
          <div className="flex-1 h-1.5 rounded-full bg-sand-200/80 overflow-hidden">
            <motion.div
          ...
          <span className="text-[10px] font-extrabold text-charcoal-500 tabular-nums">
      ...
      <div className="px-5 pt-4 pb-1 flex items-baseline justify-between">
        <h3 className="text-sm font-extrabold text-charcoal-900">{friendlyDayDate(tripStart, activeDay)}</h3>
        <span className="text-[11px] font-bold text-charcoal-400 tabular-nums">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto px-5 pb-5">
        {dayStops.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-charcoal-300 p-8 text-center text-xs font-semibold text-charcoal-400">
              ...
              return (
                <li key={stop.id} id={`journey-stop-${stop.id}`} className="relative flex gap-3 pb-1.5">
                  ...
                  <div className="flex flex-col items-center shrink-0">
                      ...
                      className={`relative z-10 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${
                        state === 'completed'
                          ...
                          : state === 'active'
                            ...
                            : 'bg-white border-charcoal-300 text-charcoal-400 hover:border-travion-400'
                      }`}
                    >
                      {state === 'completed' ? (
                    ...
                    {!isLast && (
                      <span className={`w-0.5 flex-1 min-h-6 my-0.5 rounded-full ${state === 'completed' ? 'bg-emerald-300' : 'bg-sand-200'}`} />
                    )}
                  </div>
                  ...
                  <motion.div
                    ...
                    onClick={() => handleSelect(stop)}                      className={`group flex-1 min-w-0 mb-2 rounded-2xl border px-3.5 py-3 cursor-pointer transition-all ${
                      selected || expanded
                        ? 'border-charcoal-200 bg-white shadow-soft'
                        : state === 'completed'
                          ? 'border-transparent bg-transparent opacity-70'
                          : 'border-charcoal-200/70 bg-white/60 hover:bg-white'
                    } ${isCurrent ? 'ring-2 ring-travion-200 ring-offset-1 ring-offset-ivory-50' : ''}`}
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="text-[10px] font-bold text-charcoal-400 bg-sand-100 rounded-md px-1.5 py-0.5">
                            {stop.time}
                          </span>
                          {isHere ? (
                            ...
                            <span className="text-[10px] font-bold text-charcoal-500 bg-sand-100 rounded-md px-1.5 py-0.5 inline-flex items-center gap-1">
                              <MapPin className="w-3 h-3 text-travion-500" /> {distTxt} away
                            </span>
                          ) : null}
                        </div>
                        <h4 className={`mt-1 text-[13.5px] font-extrabold leading-snug ${
                          state === 'completed' ? 'text-charcoal-400' : 'text-charcoal-900'
                        }`}>
                          {stop.title}
                        </h4>
                        <p className="mt-0.5 text-[11px] font-semibold text-charcoal-400 truncate">
                          {stop.location_name}
                        </p>
                      </div>
                      <div className="flex flex-col items-end gap-1 shrink-0">
                        <span className={`text-[11px] font-extrabold tabular-nums ${state === 'completed' ? 'text-charcoal-300' : 'text-charcoal-600'}`}>
                          {costLabel(stop.estimated_cost)}
                        </span>
                        <ChevronDown
                          className={`w-3.5 h-3.5 text-charcoal-300 transition-transform ${expanded ? 'rotate-180' : ''}`}
                        />
                      </div>
                    </div>
                    ...
                    <AnimatePresence initial={false}>
                      {(expanded || isCurrent) && (
                        <motion.div
                          ...
                          className="overflow-hidden"
                        >
                          <div className={`mt-2.5 pt-2.5 border-t ${state === 'completed' ? 'border-charcoal-100' : 'border-charcoal-100'}`}>
                            {state !== 'completed' && (
                              <p className="text-[11.5px] font-semibold text-charcoal-700 leading-relaxed">
                            ...
                            {stop.description && (
                              <p className={`mt-1.5 text-[11px] leading-relaxed ${state === 'completed' ? 'text-charcoal-400' : 'text-charcoal-500'}`}>
                                {stop.description}
                              </p>
                            )}
                            {stop.ai_note && (
                            ...
                            {stop.duration_minutes ? (
                              <p className="mt-1.5 text-[10.5px] font-bold text-charcoal-400">
                    ...
                    <div className={`flex items-center justify-between mt-2 ${expanded || isCurrent ? '' : 'opacity-0 group-hover:opacity-100'}`}>
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-charcoal-300">
        ...
        {/* Day capstone */}
        <div className="mt-1 ml-11 flex items-center gap-2 text-[11px] font-bold text-charcoal-400">
          <span className={`h-1.5 w-1.5 rounded-full ${doneInDay === dayStops.length && dayStops.length > 0 ? 'bg-emerald-400' : 'bg-sand-200'}`} />
